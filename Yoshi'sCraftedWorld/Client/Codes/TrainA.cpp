...
CTrainA::CTrainA(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CTrainA::CTrainA(const CTrainA & rhs)
	:CGameObject(rhs)
{
}

HRESULT CTrainA::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CTrainA::Ready_GameObject(void * pArg)
{
...
_int CTrainA::Update_GameObject(_double TimeDelta)
{
	m_TimeDelta = TimeDelta;

	_vec3 vPos = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	//cout << vPos.x << "//" << vPos.y << "//" << vPos.z << "// " << m_pPlayer->Get_PlayerTransform()->Get_State(CTransform::STATE_POSITION).x << "// " << m_pPlayer->Get_PlayerTransform()->Get_State(CTransform::STATE_POSITION).y << "// " << m_pPlayer->Get_PlayerTransform()->Get_State(CTransform::STATE_POSITION).z << endl;
...
_int CTrainA::LateUpdate_GameObject(_double TimeDelta)
{
	...
	if (m_pTrain->Get_IsEnd() == false)
	{
		...
		if (m_pColliderCom->Collision_Sphere((CCollider*)pManagement->Get_Component(SCENE_TRAIN, L"Layer_Object", L"Com_Collider")))
			m_pPlayer->Get_OnTrain(true) = true;
		else
			m_pPlayer->Get_OnTrain(true) = false;
...
HRESULT CTrainA::Render_GameObject()
{
...
HRESULT CTrainA::Change_CurrVec(vector<_vec3>& vPath)
{
	m_vecCurrPath = vPath;
...
HRESULT CTrainA::Add_Component()
{
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TrainA", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
...
HRESULT CTrainA::SetUp_ConstantTable()
{
...
CGameObject * CTrainA::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGameObject*		pInstance = new CTrainA(pGraphic_Device);
...
CGameObject * CTrainA::Clone_GameObject(void * pArg)
{
	CGameObject*		pInstance = new CTrainA(*this);
