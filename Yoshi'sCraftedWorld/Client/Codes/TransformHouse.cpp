#include "stdafx.h"
#include "..\Headers\TransformHouse.h"
...
CTransformHouse::CTransformHouse(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CTransformHouse::CTransformHouse(const CTransformHouse & rhs)
	:CGameObject(rhs)
{
}

HRESULT CTransformHouse::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CTransformHouse::Ready_GameObject(void * pArg)
{
	m_pTag = L"../Bin/Data/Gimmick/TownTrain/Inline/TransformHouse.ini";
	...
	m_pMeshBTransformCom->Set_State(CTransform::STATE_POSITION, _vec3(218.8f, 0.1f, 21.07f));
...
_int CTransformHouse::Update_GameObject(_double TimeDelta)
{
...
_int CTransformHouse::LateUpdate_GameObject(_double TimeDelta)
{
			...
			for (auto& pEgg : *m_pPlayer->Get_ReleasedEgg())
			{
				...
				if (m_pMeshA_CollisionCom->Collision_Mesh(vRayPos, vRayDir, fDistance))
				{
					if (fDistance <= 0.8f)
					{
						...
						if (FAILED(pManagement->Add_GameObjectToLayer(SCENE_TRAIN, L"Layer_Trigger", L"GameObject_CTrigger", (_tchar*)L"ZTrigger9_0")))
							return E_FAIL;

						if (FAILED(pManagement->Add_GameObjectToLayer(SCENE_TRAIN, L"Layer_Trigger", L"GameObject_CTrigger", (_tchar*)L"ZTrigger9_1")))
							return E_FAIL;
...
HRESULT CTransformHouse::Render_GameObject()
{
...
HRESULT CTransformHouse::Add_Component()
{
	// For.Com_Transform
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformHouse", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
	...
	//// For.Com_CollMeshA
	//if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformHouse_CollA", L"Com_CollMeshA", (CComponent**)&m_pCollisionMeshA)))
	//	return E_FAIL;

	//// For.Com_CollMeshB
	//if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformHouse_CollB", L"Com_CollMeshB", (CComponent**)&m_pCollisionMeshB)))
	//	return E_FAIL;

	if (FAILED(D3DXLoadMeshFromX(L"../Bin/Resources/Gimmicks/TownTrain/TransformHouse/SM_TransformHouseC_CollisionA.X", D3DXMESH_MANAGED, m_pGraphic_Device, nullptr, nullptr, nullptr, nullptr, &m_pCollisionMeshA)))
		return E_FAIL;

	if (FAILED(D3DXLoadMeshFromX(L"../Bin/Resources/Gimmicks/TownTrain/TransformHouse/SM_TransformHouseC_CollisionB.X", D3DXMESH_MANAGED, m_pGraphic_Device, nullptr, nullptr, nullptr, nullptr, &m_pCollisionMeshB)))
		return E_FAIL;
...
HRESULT CTransformHouse::SetUp_ConstantTable()
{
...
CGameObject * CTransformHouse::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGameObject*		pInstance = new CTransformHouse(pGraphic_Device);
...
CGameObject * CTransformHouse::Clone_GameObject(void * pArg)
{
	CGameObject*		pInstance = new CTransformHouse(*this);
