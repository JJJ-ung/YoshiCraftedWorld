...
HRESULT CHeart::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CHeart::Ready_GameObject(void * pArg)
{
...
_int CHeart::Update_GameObject(_double TimeDelta)
{
	if (m_bDead)
		return OBJDEAD;
...
_int CHeart::LateUpdate_GameObject(_double TimeDelta)
{
	...
	//		if (m_tInfo.eCoinType == CHeart::NORMAL_COIN)
	//			pUI->Add_Coin(CUI_Coin::COIN_NORMAL);
	//		else if (m_tInfo.eCoinType == CHeart::LARGE_COIN)
...
HRESULT CHeart::Render_GameObject()
{
...
CGameObject * CHeart::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGameObject*		pInstance = new CHeart(pGraphic_Device);
...
CGameObject * CHeart::Clone_GameObject(void * pArg)
{
	CGameObject*		pInstance = new CHeart(*this);
