#include "stdafx.h"
#include "..\Headers\TransformFountain.h"
...
CTransformFountain::CTransformFountain(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CGameObject(pGraphic_Device)
{
}

CTransformFountain::CTransformFountain(const CTransformFountain & rhs)
	:CGameObject(rhs)
{
}

HRESULT CTransformFountain::Ready_GameObject_Prototype()
{
	return NOERROR;
}

HRESULT CTransformFountain::Ready_GameObject(void * pArg)
{
	m_pTag = L"../Bin/Data/Gimmick/TownTrain/Inline/TransformFountain.ini";
	...
	m_pMeshBTransformCom->Set_State(CTransform::STATE_POSITION, _vec3(202.8f, 0.1f, 23.2f));
...
_int CTransformFountain::Update_GameObject(_double TimeDelta)
{
...
_int CTransformFountain::LateUpdate_GameObject(_double TimeDelta)
{
	...
	if (!m_bPass)
	{
			...
			for (auto& pEgg : *m_pPlayer->Get_ReleasedEgg())
			{
				...
				if (m_pMeshA_CollisionCom->Collision_Mesh(vRayPos, vRayDir, fDistance))
				{
					if (fDistance <= 0.8f)
					{
						...
						if (FAILED(pManagement->Add_GameObjectToLayer(SCENE_TRAIN, L"Layer_Trigger", L"GameObject_CTrigger", (_tchar*)L"ZTrigger8_0")))
							return E_FAIL;

						if (FAILED(pManagement->Add_GameObjectToLayer(SCENE_TRAIN, L"Layer_Trigger", L"GameObject_CTrigger", (_tchar*)L"ZTrigger8_1")))
							return E_FAIL;

						if (FAILED(pManagement->Add_GameObjectToLayer(SCENE_TRAIN, L"Layer_Trigger", L"GameObject_CTrigger", (_tchar*)L"ZTrigger8_2")))
							return E_FAIL;
		...
		{
			if (m_pMeshCom->Finish_Animation(0.3))
			{
				m_bPass = true;
...
HRESULT CTransformFountain::Render_GameObject()
{
...
HRESULT CTransformFountain::Add_Component()
{
	// For.Com_Transform
	...
	// For.Com_Mesh
	if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformFountain", L"Com_Mesh", (CComponent**)&m_pMeshCom)))
		return E_FAIL;
	...
	//// For.Com_CollMeshA
	//if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformFountain_CollA", L"Com_CollMeshA", (CComponent**)&m_pCollisionMeshA)))
	//	return E_FAIL;

	//// For.Com_CollMeshB
	//if (FAILED(CGameObject::Add_Component(SCENE_TRAIN, L"Component_Mesh_TransformFountain_CollB", L"Com_CollMeshB", (CComponent**)&m_pCollisionMeshB)))
	//	return E_FAIL;

	if (FAILED(D3DXLoadMeshFromX(L"../Bin/Resources/Gimmicks/TownTrain/TransformFountain/SM_TransformFountainA_CollisionA.X", D3DXMESH_MANAGED, m_pGraphic_Device, nullptr, nullptr, nullptr, nullptr, &m_pCollisionMeshA)))
		return E_FAIL;

	if (FAILED(D3DXLoadMeshFromX(L"../Bin/Resources/Gimmicks/TownTrain/TransformFountain/SM_TransformFountainA_CollisionB.X", D3DXMESH_MANAGED, m_pGraphic_Device, nullptr, nullptr, nullptr, nullptr, &m_pCollisionMeshB)))
		return E_FAIL;
...
HRESULT CTransformFountain::SetUp_ConstantTable()
{
...
CGameObject * CTransformFountain::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGameObject*		pInstance = new CTransformFountain(pGraphic_Device);
...
CGameObject * CTransformFountain::Clone_GameObject(void * pArg)
{
	CGameObject*		pInstance = new CTransformFountain(*this);
