...
HRESULT CHill::Ready_GameObject_Prototype()
{
	return NOERROR;
}
...
_int CHill::Update_GameObject(_double TimeDelta)
{
	if (true == m_pFrustumCom->Culling_OnWorldSpace(&m_pTransformCom->Get_State(CTransform::STATE_POSITION), 5.f))
	{
		m_pPlayer->Add_CollisionList(m_pCollisionCom);
		m_pPlayer->Add_HeadCollisionList(m_pCollisionCom);
	}
...
_int CHill::LateUpdate_GameObject(_double TimeDelta)
{
...
HRESULT CHill::Render_GameObject()
{
...
HRESULT CHill::SetUp_ConstantTable()
{
...
CGameObject * CHill::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CGameObject*		pInstance = new CHill(pGraphic_Device);
...
{
	CGameObject*		pInstance = new CHill(*this);
