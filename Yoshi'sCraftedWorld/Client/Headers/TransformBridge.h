...
BEGIN(Client)
class CYoshi_Stage;
class CTransformBridge final : public CGameObject
{
private:
	explicit CTransformBridge(LPDIRECT3DDEVICE9 pGraphic_Device);
	explicit CTransformBridge(const CTransformBridge& rhs);
	virtual ~CTransformBridge() = default;
